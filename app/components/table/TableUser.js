...
export default function TableUser({data}) {
    ...
    const handleSearch = async (e) =>{
        ...
        const result = users?.filter(item => item.nit.includes(search) )
        setUsers(result)
        if (search === '') {
            const users = await useUser().getData()
            setUsers(await users.user)
        }
    }

    return (<>
        <input type="text" onChange={handleSearch} className="border-2 border-gray-300 py-1 focus:border-b-2 focus:border-blue-700 transition-colors focus:outline-none peer bg-inherit" placeholder="Buscar" />
